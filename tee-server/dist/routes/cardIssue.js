...
exports.cardIssueRouter = void 0;
const express_1 = require("express");
const config_js_1 = require("../config.js");
const facilitator_js_1 = require("../x402/facilitator.js");
const issuing_js_1 = require("../stripe/issuing.js");
const encrypt_js_1 = require("../crypto/encrypt.js");
...
const types_js_1 = require("../x402/types.js");
exports.cardIssueRouter = (0, express_1.Router)();
/**
 * POST /v1/card-issue
 *
 ...
 */
exports.cardIssueRouter.post('/', async (req, res) => {
    const { amount_usd_cents, agent_pubkey, chain, nullifier_hash } = req.body;
    ...
    const paymentHeader = req.headers[types_js_1.PAYMENT_HEADER.toLowerCase()];
    // No payment yet — return 402 with requirements
    if (!paymentHeader) {
        const requirements = buildRequirements(chain, amount_usd_cents);
        return res
            .status(402)
            .set(types_js_1.PAYMENT_REQUIRED_HEADER, (0, types_js_1.encodePaymentRequired)(requirements))
            .json({
    ...
    let paymentPayload;
    try {
        paymentPayload = (0, types_js_1.decodePaymentPayload)(paymentHeader);
    }
    catch {
    ...
    const requirements = buildRequirements(chain, amount_usd_cents);
    // Verify payment
    const verifyResult = await (0, facilitator_js_1.verify)(paymentPayload, requirements);
    if (!verifyResult.valid) {
        return res.status(402).json({ error: `Payment verification failed: ${verifyResult.error}` });
    }
    // Settle payment on-chain
    const settlementResult = await (0, facilitator_js_1.settle)(paymentPayload);
    ...
    let card;
    try {
        card = await (0, issuing_js_1.createOneTimeCard)(verifiedAmountCents);
    }
    catch (err) {
    ...
    let encryptedCard;
    try {
        encryptedCard = (0, encrypt_js_1.encryptForRecipient)(agent_pubkey, cardJson);
    }
    catch (err) {
        // Cancel card since we can't deliver credentials
        await (0, issuing_js_1.cancelCard)(card.cardId);
    ...
    const receipt = {
        ...
        chain,
        issued_at: issuedAt,
        tee_pubkey: (0, signer_js_1.getTeePubkey)(),
        tx_hash: settlementResult.txHash ?? null,
    };
    const teeSignature = await (0, signer_js_1.signReceipt)(receipt);
    // Auto-cancel card after 10 minutes if not used
    setTimeout(async () => {
        await (0, issuing_js_1.cancelCard)(card.cardId);
        await (0, supabase_js_1.markCardCanceled)(card.cardId);
    }, 10 * 60 * 1000);
    // Log to audit trail
    await (0, supabase_js_1.logIssuance)({
        agent_pubkey,
        nullifier_hash,
        chain,
...
function buildRequirements(chain, amountUsdCents) {
    if (chain === 'arc-testnet') {
        ...
        return {
            ...
            payTo: config_js_1.config.arc.paymentReceiver,
            asset: config_js_1.config.arc.eurcContract,
        ...
        return {
            ...
            payTo: config_js_1.config.hedera.paymentReceiver,
